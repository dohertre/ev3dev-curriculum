...
def arm_calibration(arm_motor, touch_sensor):
    ...
    #   Command the arm_motor to run forever in the positive direction at max speed. x
    #   Create an infinite while loop that will block code execution until the touch sensor's is_pressed value is True.
    #     Within that loop sleep for 0.01 to avoid running code too fast. x
    #   Once past the loop the touch sensor must be pressed. So stop the arm motor quickly using the brake stop action. x
    #   Make a beep sound x
...
def arm_down(arm_motor):
    ...
    Type hints:
      :type arm_motor: ev3.MediumMotor
    """
    # DONE: 5. Implement the arm down movement by fixing the code below
